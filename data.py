...
for idx, row_idx in enumerate(range(1, sheet.nrows)):
	...
	entry = fill(fixture_data[home], goals_scored, L1_GOALS, L5_GOALS, L10_GOALS, SEASON_GOALS)
	fixture_sample.extend(entry)

	# -- HALFTIME GOALS --
	goals_scored_ht = row[HT_GOALS_HOME].value
	entry = fill(fixture_data[home], goals_scored_ht, L1_GOALS_HT, L5_GOALS_HT, L10_GOALS_HT, SEASON_GOALS_HT)
	fixture_sample.extend(entry)

	# -- SHOTS ON --
	shots_on = row[SHOTS_TARGET_HOME].value
	entry = fill(fixture_data[home], shots_on, L1_SHOTS_ON, L5_SHOTS_ON, L10_SHOTS_ON, SEASON_SHOTS_ON)
	fixture_sample.extend(entry)

	# -- SHOTS OFF --
	shots_off = row[SHOTS_HOME].value - row[SHOTS_TARGET_HOME].value
	entry = fill(fixture_data[home], shots_off, L1_SHOTS_OFF, L5_SHOTS_OFF, L10_SHOTS_OFF, SEASON_SHOTS_OFF)
	fixture_sample.extend(entry)

	# -- CORNERS --
	corners = row[CORNERS_HOME].value
	entry = fill(fixture_data[home], corners, L1_CORNERS, L5_CORNERS, L10_CORNERS, SEASON_CORNERS)
	fixture_sample.extend(entry)

	# -- FOULS --
	fouls = row[FOULS_HOME].value
	entry = fill(fixture_data[home], fouls, L1_FOULS, L5_FOULS, L10_FOULS, SEASON_FOULS)
	fixture_sample.extend(entry)

	# -- YELLOWS --
	yellows = row[YELLOWS_HOME].value
	entry = fill(fixture_data[home], yellows, L1_YELLOWS, L5_YELLOWS, L10_YELLOWS, SEASON_YELLOWS)
	fixture_sample.extend(entry)

	# -- REDS --
	reds = row[REDS_HOME].value
	entry = fill(fixture_data[home], reds, L1_REDS, L5_REDS, L10_REDS, SEASON_REDS)
	fixture_sample.extend(entry)

	# -- GOALS AGAINST --
	goals_scored_against = row[FT_GOALS_AWAY].value
	entry = fill(fixture_data[home], goals_scored_against, L1_GOALS_AGAINST, L5_GOALS_AGAINST, L10_GOALS_AGAINST, SEASON_GOALS_AGAINST)
	fixture_sample.extend(entry)

	# -- HALFTIME GOALS AGAINST --
	goals_scored_ht_against = row[HT_GOALS_AWAY].value
	entry = fill(fixture_data[home], goals_scored_ht_against, L1_GOALS_HT_AGAINST, L5_GOALS_HT_AGAINST, L10_GOALS_HT_AGAINST, SEASON_GOALS_HT_AGAINST)
	fixture_sample.extend(entry)

	# -- SHOTS ON AGAINST --
	shots_on_against = row[SHOTS_TARGET_AWAY].value
	entry = fill(fixture_data[home], shots_on_against, L1_SHOTS_ON_AGAINST, L5_SHOTS_ON_AGAINST, L10_SHOTS_ON_AGAINST, SEASON_SHOTS_ON_AGAINST)
	fixture_sample.extend(entry)

	# -- SHOTS OFF AGAINST --
	shots_off_against = row[SHOTS_AWAY].value - row[SHOTS_TARGET_AWAY].value
	entry = fill(fixture_data[home], shots_off_against, L1_SHOTS_OFF_AGAINST, L5_SHOTS_OFF_AGAINST, L10_SHOTS_OFF_AGAINST, SEASON_SHOTS_OFF_AGAINST)
	fixture_sample.extend(entry)

	# -- CORNERS AGAINST --
	corners_against = row[CORNERS_AWAY].value
	entry = fill(fixture_data[home], corners_against, L1_CORNERS_AGAINST, L5_CORNERS_AGAINST, L10_CORNERS_AGAINST, SEASON_CORNERS_AGAINST)
	fixture_sample.extend(entry)

	# -- FOULS AGAINST --
	fouls_against = row[FOULS_AWAY].value
	entry = fill(fixture_data[home], fouls_against, L1_FOULS_AGAINST, L5_FOULS_AGAINST, L10_FOULS_AGAINST, SEASON_FOULS_AGAINST)
	fixture_sample.extend(entry)

	# -- YELLOWS AGAINST --
	yellows_against = row[YELLOWS_AWAY].value
	entry = fill(fixture_data[home], yellows_against, L1_YELLOWS_AGAINST, L5_YELLOWS_AGAINST, L10_YELLOWS_AGAINST, SEASON_YELLOWS_AGAINST)
	fixture_sample.extend(entry)

	# -- REDS AGAINST --
	reds_against = row[REDS_AWAY].value
	entry = fill(fixture_data[home], reds_against, L1_REDS_AGAINST, L5_REDS_AGAINST, L10_REDS_AGAINST, SEASON_REDS_AGAINST)
	fixture_sample.extend(entry)
	...
	if row[FT_RESULT].value == 'H':
		...
		form = 0
	entry = fill(fixture_data[home], form, L1_FORM, L5_FORM, L10_FORM, SEASON_FORM)
	fixture_sample.extend(entry)
