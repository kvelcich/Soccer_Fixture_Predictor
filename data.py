...
for idx, row_idx in enumerate(range(1, sheet.nrows)):
	...
	# HOME_AVG_GOALS,
	if home_length == 0:
		fixture_sample.append(row[FT_GOALS_HOME].value)
	else:
		prev_total = fixture_data[home][home_length - 1][HOME_SEASON_GOALS]
		total = prev_total + row[FT_GOALS_HOME].value
	...
	if home_length == 0:
		...
		entry = l5_total - l5_game + row[FT_GOALS_HOME].value
		fixture_sample.append(entry)
	# HOME_L10_GOALS,
	if home_length == 0:
		fixture_sample.append(row[FT_GOALS_HOME].value)
		...
		entry = l10_total - l10_game + row[FT_GOALS_HOME].value
		fixture_sample.append(entry)
	# HOME_SEASON_GOALS,
	if home_length == 0:
		fixture_sample.append(row[FT_GOALS_HOME].value)
